...
import { MapComponent } from "@/components/leaflet-map-components";
  ...
  const validateCapacityInput = (value: string) => {
    try {
      ...
      if (isNaN(valueInt) || valueInt < 0) {
        ...
        setIsValidCapacity(false);
        setFormErrorMessage(
          "Capacity must be greater than or equal to the volume.",
        );
      } else {
    ...
    postLot(
      ...
      () => {
        const dialog: HTMLElement | null =
          document.getElementById("lotCreatedDialog");
        if (dialog) {
          dialog.click();
        }
      },
          ...
          <div className="flex flex-col max-w-lg gap-4 mt-5 mb-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                type="text"
                onChange={(event: ChangeEvent<HTMLInputElement>) =>
                  validateVolumeInput(event.target.value)
                }
              />
            </div>
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                type="text"
                onChange={(event: ChangeEvent<HTMLInputElement>) =>
                  validateCapacityInput(event.target.value)
                }
              />
            </div>
          </div>
